...
class CameraHandler:
    ...
    """
    def __init__(self, camera_num, resolution=None):
        self.camera_num = camera_num
        # Use configuration settings for resolution
        self.resolution = resolution or (
            camera_settings.get("resolution", {}).get("width", 640),
            camera_settings.get("resolution", {}).get("height", 480)
        )
        self.stream = True
        self.frame = None
        self.timestamp = None
...
if __name__ == "__main__":
    # Use default camera index from config
    default_camera = camera_settings.get("default_index", 0)
    camera = CameraHandler(default_camera)
    while True:
