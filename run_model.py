...
def run_model(detector_type='mediapipe', enable_drowsiness=False, show_gui=False, 
              output_json=None, camera_index=0, verbose=False):
    """
    Run the face recognition model directly without a web server
        ...
        while True:
            ...
            if verify_results:
                for result in verify_results:
                    ...
                    print(f"  User: {user_name}, Verified: {verified}, Confidence: {confidence:.2f}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run face recognition model without web server")
    parser.add_argument("-d", "--detector", type=str, default="mediapipe", 
                        choices=["mediapipe", "yolov8", "yolov8_onnx"],
                        help="Detector type (default: mediapipe)")
    parser.add_argument("-c", "--camera", type=int, default=0,
                        help="Camera index to use (default: 0)")
    parser.add_argument("-e", "--enable-drowsiness", action="store_true",
                        help="Enable drowsiness detection")
    parser.add_argument("-g", "--gui", action="store_true",
                        help="Enable GUI display (requires access to display)")
    parser.add_argument("-o", "--output", type=str,
                        help="Output JSON file to save recognition results")
